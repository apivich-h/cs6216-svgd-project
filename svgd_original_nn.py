...
class SVGDBayesNN:
    ...
    def __init__(self, X_train, y_train, batch_size=100, max_iter=1000, M=20, n_hidden=50, a0=1, b0=0.1,
        ...
        for iter in tqdm.trange(max_iter):
            ...
            for i in range(self.M):
                ...
                dw1, db1, dw2, db2, dloggamma, dloglambda = logp_gradient(X_train[batch, :], y_train[batch], w1, b1, w2,
    ...
    def evaluation(self, X_test):
        ...
        pred_y_test = np.zeros([self.M, len(X_test)])
        pred_std = np.zeros([self.M, len(X_test)])
        ...
        for i in range(self.M):
            ...
            pred_std[i, :] = np.sqrt(np.exp(-loggamma)) * self.std_y_train

        pred_mean = np.mean(pred_y_test, axis=0)
        pred_std = np.mean(pred_std, axis=0)
        return pred_mean, pred_std


def train_original_svgd(xs_train, ys_train, xs_test, hidden_dim=50, num_particles=20,
                        num_steps=5000, subsample_size=100, lr=1e-3):
    start = time.time()
    svgd = SVGDBayesNN(xs_train, ys_train, batch_size=subsample_size, n_hidden=hidden_dim, max_iter=num_steps,
                       M=num_particles, master_stepsize=lr)
    svgd_time = time.time() - start
    pred_mean, pred_std = svgd.evaluation(xs_test)
    return pred_mean, pred_std, svgd_time
