...
class PBP:
    ...
    def __init__(self, layer_sizes, mean_y_train, std_y_train):
        ...
        # We create the input and output variables in pbp_theano
        ...
        # We create a pbp_theano function for updating the posterior

        self.adf_update = theano.function([self.x, self.y], self.logZ,
                                          updates=self.network.generate_updates(self.logZ, self.logZ1,
                                                                                self.logZ2))

        # We greate a pbp_theano function for the network predictive distribution
